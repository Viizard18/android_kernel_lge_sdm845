...
#endif // CONFIG_ANDROID_LMK_NOTIFY_TRIGGER
	...
	if (selected) {
		...
		if (reclaimable_cnt && selected_task == NULL && mutex_trylock(&reclaim_mutex)) {
			...
			if (selected_task) {
				...
				rem += hswap_tasksize;
				lowmem_print(1, "Reclaiming '%s' (%d) (tgid %d), adj %hd,\n" \
						"   top time = %ld, top count %d,\n" \
						"   to free %ldkB on behalf of '%s' (%d) because\n" \
						"   cache %ldkB is below limit %ldkB for oom_score_adj %hd\n" \
						"   Free memory is %ldkB above reserved.\n",
						selected_task->comm, selected_task->pid, selected_task->tgid,
						selected_task->signal->oom_score_adj,
...
#else
		lowmem_print(1, "Killing '%s' (%d) (tgid %d), adj %hd, reclaim_cnt %d, top (%ld, %d), reason %d\n"
#endif
			"to free %ldkB on behalf of '%s' (%d) because\n"
